...
function createPullQuoteV2(main, document) {
    const primarySections = document.querySelectorAll('.callout.ctr.bg-blue, .callout.f_rt.bg-blue, blockquote .callout, blockquote');
    primarySections.forEach(primarySection => {
        if (primarySection) {
            ...
            primarySection.querySelectorAll('h1, h2, h3, h4, h5, h6, p').forEach(element => {
            ...
            const cells = [
                ['pullquote-v2'],
                [contentHTML.trim()],
                [quoteeNameHTML ? quoteeNameHTML.trim() : '', quoteeHTML ? quoteeHTML.trim() : '']
            ];
...
function removeTextFromSrOnly(document) {
    const wordsToRemove = ['External Site', 'External Link', 'Send Email', 'Secure Site', ' Secure Site', 'Opens PDF', 'PDF File', 'Secure'];
...
function removeImageDesc(document) {
    ...
    infographics.forEach(infographic => {
        ...
        buttons.forEach(button => {
            const ariaControls = button.getAttribute('aria-controls') || button.getAttribute('ariacontrols');
            if (ariaControls) {
